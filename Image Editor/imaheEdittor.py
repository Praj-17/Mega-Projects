...
def selected():
    ...
    Canvas2.creat_image(300,210,image=img1)
    Canvas2.image=img1

def blur(event):
    global img_path, img1, imgg
    for i in range(0,v1.get()+1):
        img = Image.open(img_path)
        img.thumbnail((350,350))
        imgg = img.filter(ImageFilter.BoxBlur(i))
